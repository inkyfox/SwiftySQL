...
class SQLUpdateGenerator: SQLQueryGenerator<SQLUpdate> {
    
    func actionString(_ orAction: SQLUpdate.OrAction?) -> String {
    ...
    override func generate(_ element: SQLUpdate, forRead: Bool) -> String {
        return generateQuery(element, forRead: forRead)
    }
    
    override func generateFormatted(_ element: SQLUpdate,
                                    forRead: Bool,
                                    withIndent indent: Int) -> String {
        return generateFormattedQuery(element, forRead: forRead, withIndent: indent)
    }
    
    override func generateQuery(_ element: SQLUpdate, forRead: Bool) -> String {
    ...
    override func generateFormattedQuery(_ element: SQLUpdate,
                                         forRead: Bool,
                                         withIndent indent: Int) -> String {
        var query = actionString(element.orAction) + " "
        ...
        query +=
            element.sets
                ...
                .map { (columns, values) -> String in
                    ...
                    let needNewLine = values.count > 1 || values[0] is SQLSelect
                    let rhsIndent: Int
                    if needNewLine {
                        query += " =\n"
